...
export default class User extends Component {
    ...
    constructor(props) {
        super(props)
        this.state = {
            ip_server: 'http://3.0.56.213:5000/api/users',
            loading: true,
            data: [],
            isFetching: false,
            token: ''
            ...
            <TouchableOpacity style={{flex:1, flexDirection: 'row', backgroundColor: 'transparent', justifyContent: 'center'}}>
                    <View style = {styles.containerList}>
                        <Icon
                            ...
                            color = '#EB615B' />
                        {/*<Text style={styles.itemHeader}>{(data.item.nama_User.length > 30) ? data.item.nama_User.substring(0, 28) + '..' : data.item.nama_User}</Text>*/}
                        <Text style={styles.itemHeader}>test</Text>
                    </View>
                </TouchableOpacity>
        )
    }
